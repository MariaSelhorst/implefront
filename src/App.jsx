...
function App() {
  ...
  const getUsers = async () => {
    try {
      const res = await axios.get("http://implementa-o-1.onrender.com/api/person");
      setList(res.data);
  ...
  const registerUser = async () => {
    try { 
      await axios.post("http://implementa-o-1.onrender.com/api/person",{
  ...
  const deleteUser = async (id) => {
    try { 
      await axios.delete(`http://implementa-o-1.onrender.com/api/person/${id}`);
      getUsers()
    } catch (error) {
      console.error("Erro ao inserir", error);
    }
